...
```{r formating error while reading, error=TRUE, eval=TRUE}
...
```{r input error while reading, error=TRUE, eval=TRUE}
...
In our context, the Prevosti's distance is a measure of the mean absolute difference in allele frequencies across all alleles and loci between 2 populations. It is defined as $D_{\mathrm{pop.x} \ \mathrm{pop.y}} = \frac{1}{2\times\mathrm{nLoc}}\sum^{\mathrm{nLoc}}_{j = 1}\sum^{\mathrm{nAlleles}_j}_{i = 1}|x_{ij} - y_{ij}|$, where $x_{ij}$ is the frequency of the allele $i$ at the locus $j$ in population $x$ and where $y_{ij}$ is the frequency of the allele $i$ at the locus $j$ in population $y$. 

We again restrict our analysis to `Pop1` to begin with:
...
***Why is the allele BM2113.132 so influential?***
