...
This `poppr` function randomly samples loci without replacement and counts the number of multilocus genotypes observed.
...
However, it is not activelly maintained.
...
To answer this question, we use `seppop()` and `lapply()` as before, but now we include the function that we want to run on eahc pop (here, `ia()`) and the arguments for such a function.
...
This measure does not make assumptions about mutation, genetic drift, etc.
...
This is particularly useful to perform assignements of samples from unknonw origins to populations!

Unfortunatelly, certain steps cannot work with missing data.
...
Note that for a relativelly small number of cluster like here, `n.da` does not have to be anything else than `Inf`.
...
To figure this out, we need to go back to the clustering we starting with and compare it to our populations:
