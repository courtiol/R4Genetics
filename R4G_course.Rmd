...
-   `*italic*` -\> *italic* (1 underscore on each side)
-   `*bold*` -\> **bold** (2 underscores on each side)
-   `**both**` -\> ***both*** (3 underscores on each side)
...
As you will see, the `genind` object is a particular kind of list. Technically it is called a S4 object. Instead of elements accessible with `$`, S4 objects have slots accessible with `@` (even if the programmer behind `adegenet` made it possible to use `$` anyway). \_\_\_
...
```{r droping mlg}
myData <- myData[!indNames(myData) %in% c("N468", "N450","N463")]
myData
```
...
## *Do you remember ...* {.tabset .tabset-pills}
...
unlist(lapply(seppop(myData), nInd)) ## careful during your interpretation: pop sizes differ!
```
...
We can see that LD is not the same for all pairs of loci. Or can we? (Dan cannot!)
...
As any estimate, the $F_{ST}$ is measured with some uncertainty. To represent this uncertainty, we can compute the 95$\%$ confidence interval of the mean $F_{ST}$ value using a "very simple" bootstrap (more advanced technics are possible):
...
```{r nb of alleles vs pop size}
plot(summary(myData)$pop.n.all ~ summary(myData)$n.by.pop,
     xlab = "Pop size", ylab = "Number of alleles", las = 1,
     col = NULL)
text(x = summary(myData)$n.by.pop,
     y = summary(myData)$pop.n.all,
     labels = attr(summary(myData)$n.by.pop, "name"))
```
...
An alernative to neighbour joining is to reduce the dimensionality of the problem so that it can be plotted in 2 dimensions instead of one dimension per locus. Many technics exists but the most common one is the so-called Principal Component Analysis, or PCA for short.
